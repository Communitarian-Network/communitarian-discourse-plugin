...
RSpec.describe User, type: :model do
  ...
  let(:topic) { Fabricate(:topic, is_resolution: false) }
  ...
  describe ".posted_after?" do
    ...
    context "when there is a fresh post in the topic by the user" do
      let(:post) { Fabricate(:post, user: user, created_at: 3.minutes.ago, topic: topic) }

      it { is_expected.to eq true }
    end

    context "when there is an old post in the topic by the user" do
      let(:post) { Fabricate(:post, user: user, created_at: 7.minutes.ago, topic: topic) }
